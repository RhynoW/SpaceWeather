...
| 作法 | 適用 | 理由 |
|---|---|---|
| **MP4**（`st.video`） | 高解析度序列 | SUVI 304Å 單幀 1.1 MB × 359 幀 = **397 MB**，瀏覽器逐幀載入不切實際；編碼後同內容 12 MB。用 NASA 自產的 MP4（SDO 304、LASCO C2） |
...
| 頁面 | 用途 |
|---|---|
| **值勤模式** | **一屏掌握全局**：三網域燈號＋最近事件卡＋資料齡期＋關鍵指標判讀 |
...
| **太陽與行星際影像** | 11 張公開影像＋**4 段動畫**：黑子／日珥／磁圖／日冕儀、CME 傳播模擬、D 層吸收、全球 TEC、極光橢圓 |
| **名詞與判讀** | **教育推廣用**：因果鏈、各參數現值落在哪一區、常見誤讀 |
| **使用指南** | 依讀者的問題編排的 app 內說明，含完整來源標註表 |
| **STEM 教學** | **12–18 歲教學頁**，四語（繁中／日本語／English／Bahasa Melayu）＋三個互動小遊戲 |
