...
實測 2023-02 至 2026-06（10 個事件窗，n=799），依 **ap** 分層：

    ap < 10    n=343   中位 0.80   1σ(log) 0.223
    ap 10–20   n=227   中位 0.80   1σ(log) 0.251
    ap 20–50   n=151   中位 0.79   1σ(log) 0.232
    ap >= 50   n= 78   中位 0.85   1σ(log) 0.282

**中位數在各 ap 帶幾乎持平，隨活動度成長的是散布。**

> 這推翻了本檔早期的結論。當時只有 Gannon 一個事件窗（n=83）且**以 observed
> 分層**，得到「比值隨增強倍數單調上升 0.85→1.70，即 MSIS 暴時響應被壓縮」。
> 擴到 10 個事件窗並改以 ap 分層後，那個上升消失了——它是分層方式加上小樣本
> 的產物。改以 ap 分層的理由見 `summarize()`：用被檢驗的量本身分組，
> 恢復期會被錯分。

**能宣稱的是散布，不是偏差。** 兩條基線（觀測側的 F10.7 迴歸與模式側的
storm_ratio）之間可能存在常數偏移，該偏移會平移整欄比值，
故中位數 0.80 **不可**引用為「MSIS 高估 20%」。但常數偏移不改變散布，
所以 1σ 是可以宣稱的量——`tools/calibrate_density_uncertainty.py` 據此
產生 `docs/density_calibration.json`，取代原本手訂的不確定度常數。
