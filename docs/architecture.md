...
| 五 多頻段訊號影響 | §9.3 | 新建 C5；**軌道預報一列已具備**；HF 分項因 D-RAP 現成可用而可提前動工；**GNSS／RTK 一列已具備**（e-GNSS I95，configs/matrix/rtk.yaml） |
...
**2026-08-26 的補充**：e-GNSS 的 I95 應從 C 級移到 **B 級**——公開頁面即可取得
（連接器已實作並納入自動更新），卡住的是使用授權而非技術，且授權已在申請中。它同時補上了本案
GNSS 網域最關鍵的缺口：I95 是唯一量到「差分電離層延遲」的判據，
也是唯一有作業單位公告門檻的判據。詳見 docs/rtk_ionosphere.md。
...
6. **e-GNSS I95 使用授權**（內政部國土測繪中心）——**申請中**，
   經夏漢民太空中心協助提出。因無回填管道（歷史只能自開始輪詢之日起累積），
   本案已先行納入自動更新；核准前原始圖檔與衍生數值不進公開版控、
   不納入 data/demo。待確認的是核准後能否取得**官方數值資料**而非圖表
   ——若可，圖表擷取模組應被取代而非改良。
7. I95 的正式定義與單位（是否為 ppm 尺度、95 百分位的統計窗長、是否已正規化到
   某個基線長度）。門檻已採官方公告值（8／20／30），定義未核對前
   規則檔維持 `calibrated: false`。
8. 金門與澎湖是否需要各自的 RTK 規則集（目前只判本島 VRS 網）。
