...
**1. 48 小時預報未勝過基線，與文獻描述的困難度一致——但這不免除實作責任。**
...
**但不可就此把未達標全歸因於物理可預報性。**
本案同時存在明確的過擬合（訓練折 POD 0.83、測試折 0.38–0.02），
且以下可能成因均尚未逐一排除：特徵不足（見 §2.4 前驅量、§2.5 耦合函數）、
事件樣本稀少、機率未校準（見 §5.2）、操作點與目標變數選擇、模型架構限制。

→ 正確的表述是：**在本案目前的資料、特徵、模型與驗證設定下未達可作業門檻**。
對計畫管理的意涵是：「48 小時 POD ≥ 0.7」以目前配置不具足夠證據支持，
宜與需求單位就**事件定義、時間容差窗與比較基準**重新協商，
而非只調整數值目標、也不是停止改進實作。〔文獻〕
